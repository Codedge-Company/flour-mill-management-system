...
const createSale = async ({ customer_id, payment_method, items }, user) => {
  ...
  // Validate stock first (no transaction needed)
  ...
  // Update inventory (no transaction - sequential)
  for (const ri of resolvedItems) {
    const inventory = await Inventory.findOne({ pack_type_id: ri.pack_type_id });
    const newQty = inventory.stock_qty - ri.qty;
    await Inventory.findOneAndUpdate(
      { pack_type_id: ri.pack_type_id },
      { stock_qty: newQty, last_updated_at: new Date() }
    );
    await triggerLowStockNotification(ri.pack_type_id, newQty, null);
  }
...
const getAllPaginated = async (page = 0, size = 20, filters = {}) => {
  ...
  const match = { status: { $ne: 'DRAFT' } }; // ✅ Pure JavaScript
...
module.exports = { getAllPaginated, getAll, getById, createSale, cancelSale, remove };
