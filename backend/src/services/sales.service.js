...
const createSale = async ({ customer_id, payment_method, sale_datetime, items, use_default_price = false }, user) => {
    ...
    for (const item of items) {
        const { pack_type_id, qty, unit_price_sold: manualPrice } = item;
...
const getAllPaginated = async (page = 0, size = 20, filters = {}) => {
    ...
    const isPendingFilter = filters.paymentStatus === 'PENDING';
        ...
        Sale.aggregate([
            ...
            },
            {
                $group: {
                    ...
                    total_cost: {
                        $sum: { $multiply: [isPendingFilter ? '$pendingRatio' : '$payRatio', '$total_cost'] },
                    },
                    total_profit: {
                        $sum: { $multiply: [isPendingFilter ? '$pendingRatio' : '$payRatio', '$total_profit'] },
                    },
                },
            },
        ]),
