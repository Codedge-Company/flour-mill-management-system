...
const { JWT_SECRET } = require('./src/config/config'); // ✅ same as auth.middleware.js
...
io.use((socket, next) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded) => {  // ✅ JWT_SECRET from config
    if (err) {
      console.error('[Socket] JWT verify failed:', err.message);
      return next(new Error('Invalid token'));
    }

    // ✅ your JWT payload uses user_id field
...
io.on('connection', (socket) => {
  ...
  socket.join(`user_${socket.userId}`);  // private room per user
